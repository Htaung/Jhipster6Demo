...
public class PostResource {
    ...
    public ResponseEntity<?> updatePost(@Valid @RequestBody Post post) throws URISyntaxException {
        ...
        if (!post.getBlog().getUser().getLogin().equals(SecurityUtils.getCurrentUserLogin().orElse(""))) {
    ...
    public ResponseEntity<?> getPost(@PathVariable Long id) {
        ...
        if (post.isPresent() && post.get().getBlog().getUser() != null && !post.get().getBlog().getUser().getLogin().equals(SecurityUtils.getCurrentUserLogin().orElse(""))) {
            return new ResponseEntity<>("error.http.403", HttpStatus.FORBIDDEN);
        }
